...
const BookDetailModal = ({ bookId, isOpen, onClose }) => {
  ...
  const handleSave = () => {
    if (!formData.title || !formData.title.trim()) { showToast('Kitap baÃ…Å¸lÃ„Â±Ã„Å¸Ã„Â± boÃ…Å¸ olamaz.', 'error'); return; }
      ...
      <div className="bg-white rounded-t-3xl sm:rounded-2xl w-full max-w-md overflow-hidden shadow-2xl flex flex-col max-h-[90vh] sm:h-auto animate-in slide-in-from-bottom-10">
        <div className="p-4 border-b flex justify-between items-start bg-zinc-50 relative gap-3">
          ...
          <div className="flex-1 pr-4 min-w-0">
            ...
            <p className="text-xs text-zinc-500">ISBN: {book.isbn || 'BelirtilmemiÃ…Å¸'}</p>
          </div>
        ...
        <div className="flex border-b divide-x divide-zinc-100 bg-white">
          <button onClick={toggleLibrary} className={`flex-1 py-3 px-2 flex flex-col items-center gap-1.5 transition-colors ${book.inLibrary ? 'bg-zinc-900 text-white' : 'text-zinc-500 hover:bg-zinc-50'}`}><Library size={18} /><span className="text-[10px] uppercase font-bold tracking-wider">KÃƒÂ¼tÃƒÂ¼phanemde</span></button>
        ...
        <div className="p-5 flex-1 overflow-y-auto space-y-4">
          ...
          {!isEditing && (
            <div className="pt-2">
              <button onClick={() => setShowMove(!showMove)} className="flex items-center gap-2 text-sm text-zinc-600 hover:text-zinc-900 font-medium py-2"><MoveRight size={16} /> KlasÃƒÂ¶rÃƒÂ¼ DeÃ„Å¸iÃ…Å¸tir</button>
              {showMove && (
        ...
        <div className="p-4 border-t border-zinc-100 bg-white flex flex-col gap-2">
          {showDeleteConfirm ? (
            <div className="bg-red-50 p-3 rounded-xl border border-red-100 flex flex-col gap-2">
              <p className="text-xs text-red-800 font-medium flex items-center gap-1.5"><AlertCircle size={14}/> Bu kitabÃ„Â± silmek istediÃ„Å¸inize emin misiniz?</p>
              <div className="flex gap-2">
                <button onClick={() => setShowDeleteConfirm(false)} className="flex-1 py-2 bg-white text-zinc-600 text-sm font-medium rounded-lg border border-zinc-200">Ã„Â°ptal</button>
                <button onClick={handleDelete} className="flex-1 py-2 bg-red-600 text-white text-sm font-medium rounded-lg">Evet, Sil</button>
              </div>
            </div>
          ) : isEditing ? (
            <div className="flex gap-2">
              <button onClick={() => { setFormData(book); setIsEditing(false); }} className="flex-1 py-2.5 rounded-xl border border-zinc-200 text-zinc-600 font-medium hover:bg-zinc-50 transition-colors">Ã„Â°ptal</button>
            ...
            <div className="flex gap-2">
              <button onClick={() => setShowDeleteConfirm(true)} className="p-2.5 rounded-xl border border-zinc-200 text-red-500 hover:bg-red-50 transition-colors shrink-0" title="KitabÃ„Â± Sil"><Trash2 size={20} /></button>
              <button onClick={() => setIsEditing(true)} className="flex-1 py-2.5 rounded-xl flex items-center justify-center gap-2 bg-zinc-900 text-white font-medium hover:bg-zinc-800 transition-colors">Bilgileri DÃƒÂ¼zenle</button>
            </div>
          )}
        </div>
      </div>
